...
const candidates = firestore.collection('people');
...
//==============================================================================
/**
 * Endpoint for fetching Candidates Details. Supports only GET and OPTIONS.
 */
exports.candidates = functions.https.onRequest((req, res) => {
      ...
      if (querySnapshot.empty) {
        return people
          ...
          .catch(err => {
            ...
            return res
              .status(500)
              .send({ message: 'Unable to store details.' });
          });
      } else {
        // This is a duplicate email, so return a conflict error
        return res
          .status(409)
          .send({ message: newRecord.email + ' already exists.' });
      }
    })
    .catch(err => {
      console.error(err);
